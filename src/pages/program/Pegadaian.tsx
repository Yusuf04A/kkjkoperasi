...
export const Pegadaian = () => {
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      ...
      try {
        ...
        // Fallback: tetap gunakan file asli jika kompresi error
        setImageFile(file);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Mengunggah file yang sudah dikompres
  ...
  const executeRedeem = async () => {
    ...
    try {
      if ((user?.tapro_balance || 0) < totalPay) throw new Error("Saldo tidak mencukupi.");
          ...
          <div className="bg-white rounded-[2rem] shadow-sm border border-green-50 p-6 lg:p-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1 flex items-center gap-2"><Camera size={14} className="text-[#136f42]" /> Foto Barang Emas (Otomatis Kompres)</label>
