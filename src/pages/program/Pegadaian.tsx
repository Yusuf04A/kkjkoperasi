...
export const Pegadaian = () => {
  ...
  // State Form Pengajuan (Default Tenor 4 Bulan)
  ...
  // State Transaksi Tebus
  const [itemToRedeem, setItemToRedeem] = useState<any>(null);
  const [showRedeemDetails, setShowRedeemDetails] = useState(false); // Pop-up Rincian
  const [showPinModal, setShowPinModal] = useState(false); // Pop-up PIN
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from('pawn_transactions').insert({
        ...
        tenor_bulan: parseInt(formData.tenor), // Simpan Tenor
        image_url: imageUrl,
        status: 'pending'
      });
  ...
  const handleOpenRedeem = (item: any) => {
    ...
    setShowRedeemDetails(true); // Buka rincian dulu
  };

  const proceedToPin = () => {
    // Hitung total bayar (Pokok + 5% Admin)
    // 0.05 = 5%
    ...
    setShowPinModal(true); // Lanjut ke PIN
  };

  const executeRedeem = async () => {
    if (!itemToRedeem) return;

    // HITUNG TOTAL YANG HARUS DIBAYAR (POKOK + 5%)
    ...
    try {
      // 1. Cek Saldo Lagi (Server side check simulation)
      if ((user?.tapro_balance || 0) < totalPay) {
        throw new Error("Saldo tidak mencukupi.");
      }

      // 2. Potong Saldo (Total Pay)
      ...
      await supabase.from('transactions').insert({
        user_id: user?.id,
        type: 'withdraw',
        amount: totalPay, // Simpan total yang dibayar
        status: 'success',
        description: `Tebus Gadai: ${itemToRedeem.item_name} (Pokok + Jasa 5%)`
      });
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-center">
                    ...
                    <div className="flex justify-between items-center border-t border-green-50/50 pt-3">
                      ...
                      {item.status === 'approved' && (
                        <button 
                          onClick={() => handleOpenRedeem(item)} // KLIK TEBUS -> BUKA DETAIL
            ...
            <div className="space-y-6">
              ...
              <div className="flex items-center gap-4 bg-gray-50 p-4 rounded-2xl border border-gray-100">
                ...
                <div>
                  ...
                  <span className="text-[10px] bg-blue-50 text-blue-700 px-2 py-0.5 rounded font-bold mt-1 inline-block">
                    Tenor: {itemToRedeem.tenor_bulan || 4} Bulan
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between text-sm">
                  <div className="flex items-center gap-2">
                    <span className="text-gray-500">Jasa Titip / Admin (5%)</span>
                  </div>
                  <span className="font-bold text-gray-900">
                    {formatRupiah(itemToRedeem.loan_amount * 0.05)}
                  </span>
                </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  {/* Total: Pokok + (Pokok * 0.05) */}
                  <span className="font-black text-[#136f42] text-xl tracking-tight">
                    {formatRupiah(itemToRedeem.loan_amount + (itemToRedeem.loan_amount * 0.05))}
                  </span>
                </div>
              </div>
              ...
              <div className="bg-blue-50 p-4 rounded-xl flex gap-3 border border-blue-100">
                ...
                <p className="text-[11px] text-blue-800 leading-relaxed">
                  Pembayaran akan dipotong langsung dari <b>Saldo Tapro</b> Anda. Pastikan saldo mencukupi.
                </p>
              </div>

              <button 
                onClick={proceedToPin}
                className="w-full bg-[#136f42] text-white py-4 rounded-xl font-bold text-sm uppercase tracking-widest shadow-lg shadow-green-900/20 active:scale-95 transition-all hover:bg-[#0f5c35]"
              >
                Konfirmasi & Bayar
              </button>
            </div>
