...
export const CheckoutBelanja = () => {
    ...
    return (
        <div className="min-h-screen bg-gray-50 pb-12 font-sans text-slate-900 text-left lowercase">
            <div className="bg-[#136f42] text-white p-6 rounded-b-[2.5rem] shadow-md">
                <div className="flex items-center gap-3">
                    <button 
                        onClick={() => navigate(-1)} 
                        className="p-2 bg-white/10 rounded-xl hover:bg-white/20 transition-all shrink-0 uppercase"
                    >
                        <ArrowLeft size={22} />
                    </button>
            ...
            <div className="max-w-xl mx-auto px-6 -mt-4 space-y-6">
                <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-green-100 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-lg font-black text-slate-900 uppercase">{formatRupiah(user?.tapro_balance || 0)}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-[2.5rem] p-8 shadow-sm border border-green-100 space-y-6">
                    ...
                    <div className="space-y-4 max-h-60 overflow-y-auto no-scrollbar uppercase">
                        {cart.map((item: any) => (
                            <div key={item.product.id} className="flex justify-between items-center">
                                <div className="flex items-center gap-3">
                                    ...
                                    <p className="text-xs font-bold text-slate-700 tracking-tighter">{item.product.name}</p>
                                </div>
                                <p className="text-xs font-black text-slate-900">{formatRupiah(item.product.price * item.quantity)}</p>
                            </div>
                        ))}
                    </div>
                    <div className="pt-6 border-t-2 border-dashed border-green-100 flex justify-between items-center uppercase">
                ...
                <div className="bg-white rounded-[2.5rem] p-8 shadow-md text-center border-t-4 border-amber-400 relative overflow-hidden">
                    ...
                    <p className="text-[9px] text-slate-400 font-medium italic mt-4 relative z-10 first-letter:uppercase">pesanan akan divalidasi oleh admin sebelum saldo tapro anda dikurangi.</p>
                </div>
            </div>
