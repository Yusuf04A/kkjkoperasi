...
Terima kasih. 🙏
    ...
    // FIX SISA POKOK (Agar tidak NaN)
    // Ambil remaining_amount, jika null/undefined jadikan 0
    const sisaPokok = Number(loan.remaining_amount ?? 0);

    return (
        <div className="p-4 md:p-6 max-w-6xl mx-auto min-h-screen bg-gray-50/50">

            {/* HEADER SIMPLE (Sama seperti Data Transaksi) */}
            <div className="mb-6">
                ...
                <div className="flex items-center justify-between">
                    ...
                    <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide border ${loan.status === 'active' ? 'bg-blue-50 text-blue-600 border-blue-200' :
                            loan.status === 'paid' ? 'bg-green-50 text-green-600 border-green-200' : 'bg-gray-100 text-gray-600 border-gray-200'
                        }`}>
                        STATUS: {loan.status === 'active' ? 'AKTIF' : loan.status}
                    </span>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                ...
                <div className="lg:col-span-4 space-y-5">
                    ...
                    <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div className="overflow-hidden">
                                <h3 className="font-bold text-gray-900 truncate">{loan.profiles?.full_name}</h3>
                                <p className="text-xs text-gray-500 flex items-center gap-1">
                                    <Smartphone size={12} /> {loan.profiles?.phone || '-'}
                                </p>
                            </div>
                        </div>
                        {/* Tombol WA */}
                    ...
                    {/* 3. ALERT PERPANJANGAN (Kondisional) */}
                ...
                {/* === KOLOM KANAN (DAFTAR CICILAN) - SPAN 8 === */}
