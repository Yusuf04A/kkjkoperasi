                    ...
                    <div key={dist.id} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex flex-col md:flex-row gap-8 hover:shadow-md transition-shadow">
                        <div className="flex-1 space-y-4">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 bg-slate-50 p-6 rounded-2xl border border-slate-100 shadow-inner">
                                <div><p className="text-[9px] text-slate-400 font-black mb-1">Laba Kotor</p><p className="text-sm font-black text-slate-800">{formatRupiah(dist.gross_profit)}</p></div>
                                <div><p className="text-[9px] text-slate-400 font-black mb-1">PPH (0.5%)</p><p className="text-sm font-black text-rose-500">-{formatRupiah(dist.pph_amount)}</p></div>
                                <div><p className="text-[9px] text-slate-400 font-black mb-1">Operasional</p><p className="text-sm font-black text-slate-800">{formatRupiah(dist.operational_cost)}</p></div>
                                <div><p className="text-[9px] text-slate-400 font-black mb-1">LHU Anggota</p><p className="text-lg font-black text-[#136f42] tracking-tighter">{formatRupiah(dist.total_lhu_member)}</p></div>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col justify-center gap-3 md:border-l md:pl-8 border-slate-100 min-w-[220px]">
                            {dist.status === 'waiting' ? (
                                <>
                                    <button onClick={() => setConfirmModal({ isOpen: true, type: 'approve', dist })} className="w-full py-4 bg-[#136f42] text-white rounded-xl font-black text-[10px] tracking-[0.2em] shadow-lg shadow-green-900/10 active:scale-95 transition-all flex items-center justify-center gap-2"><CheckCircle size={18} /> Setujui & Cairkan</button>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in">
                    <form onSubmit={handleGenerateLHU} className="bg-white w-full max-w-md rounded-[2.5rem] p-10 shadow-2xl space-y-8 animate-in zoom-in-95 border border-white/20">
                        <div className="flex justify-between items-center border-b pb-4">
                            <h2 className="text-xl font-black text-slate-900 tracking-tight leading-none">Generate LHU</h2>
            ...
            {confirmModal.isOpen && (
                <div className="fixed inset-0 z-[200] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in">
                    <div className="bg-white w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl relative animate-in zoom-in-95 duration-200 border border-white/20 text-center">
                        ...
                        <h3 className="text-lg font-black text-slate-800 tracking-tight mb-2">{confirmModal.type === 'approve' ? 'Eksekusi LHU' : 'Hapus Estimasi'}</h3>
