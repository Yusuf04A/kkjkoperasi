        ...
        try {
            ...
            canvas.toBlob(async (blob) => {
                ...
                if (navigator.share) {
                    ...
                    toast.error("Browser tidak support share. Gunakan fitur Simpan.", { id: toastId });
                }
            });
        } catch (err) {
                ...
                <div className="max-w-xl mx-auto px-4 relative z-10">
                    ...
                    >
                        ...
                        <div className="px-5 py-2 flex justify-between items-center relative z-10 gap-4 flex-1">
                            ...
                            <div className="w-24 h-32 md:w-28 md:h-36 bg-gray-200 rounded-md border-[3px] border-white shadow-lg overflow-hidden flex-shrink-0 relative">
                                <img
                                    src={user?.avatar_url || `https://ui-avatars.com/api/?name=${userData.name}&background=EF4444&color=fff&size=200&font-size=0.35`}
                                    alt="Foto Member"
                    ...
                    <div className="flex justify-end gap-3 mt-3 px-2">
                        <button
                            ...
                            className="flex items-center gap-2 bg-white/10 hover:bg-white/20 text-blue-50 border border-white/20 px-4 py-2 rounded-full text-xs font-bold transition-all active:scale-95"
                        >
                        ...
                        <div className="grid grid-cols-4 gap-4">
                            {quickActions.map((action) => (
                                <Link key={action.label} to={action.link} className="flex flex-col items-center gap-2 group cursor-pointer">
                                    ...
                                    <span className="text-[10px] font-medium text-gray-600 group-hover:text-kkj-blue text-center leading-tight">{action.label}</span>
                                </Link>
                            ))}
                        </div>
                    </div>
